...
class LoginViewController: UIViewController {
    ...
    private func continueLoginFlow(email: String, password: String){
        if accountState == .existingUser{
            authSession.signExistingUser(email: email, password: password) { [weak self]
                (result) in
                switch result {
            ...
            authSession.createNewUser(email: email, password: password) { (result) in
                ...
                case .success(let authDataResult):
                    DispatchQueue.main.async {
                        self.errorLabel.text = "Hope you enjoy our app experiene. Email used \(authDataResult.user.email ?? "")."
                        self.errorLabel.textColor = .systemGreen
                    }
                }
            }
